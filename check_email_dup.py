...
DEFAULT_WORKBOOK_WORKERS = 1000
DEFAULT_WORKBOOK_CHUNK = 1000

# WHOIS: cap parallel TCP connections (align with typical workbook worker count); retries in whois_domain_age_ok.
_WHOIS_CONCURRENCY = threading.Semaphore(128)
_WHOIS_RETRIES = 3
_SMTP_RCPT_CONCURRENCY = threading.Semaphore(64)
...
async def run_duplicate_check_phase(
    ...
    *,
    ...
    pending_mask = passed_mask & unchecked_mask
    if only_row_indices is not None:
        allowed_df_idx = {r - 2 for r in only_row_indices if int(r) >= 2}
        pending_mask = pending_mask & df.index.isin(allowed_df_idx)
    pending_indices = df[pending_mask].index[:batch_size].tolist()
...
async def run_workbook_validation(
    ...
    *,
    ...
    If ``only_row_indices`` is set, only **Excel row numbers** (1-based, including header row 1)
    in this set are considered for validation; all other data rows are skipped for this run.
    """
    if not input_path.is_file():
        raise SystemExit(f"Input file not found: {input_path}")
    ...
    try:
        ...
        if only_row_indices is not None:
            sample = sorted(only_row_indices)
            tail = "..." if len(sample) > 24 else ""
            print(
                f"Row filter: only {len(only_row_indices)} Excel row number(s) "
                f"({', '.join(str(x) for x in sample[:24])}{tail})",
                flush=True,
            )
        ...
        for r in range(1, max_row):
            row_idx = r + 1
            if only_row_indices is not None and row_idx not in only_row_indices:
                continue
