...
def _scoped_sub_keys_for_parent_hint(industry_hint: Optional[str]) -> Optional[List[str]]:
    """When ``industry_hint`` maps to taxonomy parent(s), narrow sub-key search for definition match."""
    h = (industry_hint or "").strip()
    if not h:
        return None
    try:
        from industry_taxonomy import INDUSTRY_TAXONOMY
        from lead_cache_json.industry_normalizer import resolve_taxonomy_sub_key_scope

        scope = resolve_taxonomy_sub_key_scope(allowed_parent_industries=[h])
        if not scope:
            return None
        if len(scope) >= max(1, int(len(INDUSTRY_TAXONOMY) * 0.92)):
            return None
        return list(scope)
    except ImportError:
        return None
...
def _build_industry_classification_brief(
    description: Optional[str],
    candidates: List[str],
    *,
    max_chars: int = _TAXONOMY_DEFINITION_COMPARE_MAX_CHARS,
) -> str:
    """Merge page description with candidate strings so definition similarity has enough signal."""
    parts: List[str] = []
...
def _pick_sub_industry_from_taxonomy_definition_similarity(
    ...
    *,
    ...
    (similarity ``>= min_ratio``). Uses parent-scoped sub keys when ``industry_hint`` resolves to
    taxonomy parents; otherwise searches the full taxonomy.
    """
    try:
        from lead_cache_json.industry_normalizer import pick_sub_industry_from_definition_similarity

        scope = _scoped_sub_keys_for_parent_hint(industry_hint)
...
def enrich_company_web_profile(
    ...
    *,
    ...
    if not _env_skip_industry_taxonomy():
        brief_for_class = _build_industry_classification_brief(description_v, refine_candidates)
        if not (sub_out or "").strip():
            guessed_sub = _pick_sub_industry_from_taxonomy_definition_similarity(
                brief_for_class, ind_out
            )
            if guessed_sub:
                sub_out = guessed_sub
        if not (sub_out or "").strip():
            try:
                ...
                ci, cs, _err = classify_industry_sub_from_description_in_scope(
                    brief_for_class,
                    allowed_parent_industries=[ind_out] if (ind_out or "").strip() else None,
                    industry_hint=ind_out,
                )
                if ci and cs:
                    ind_out, sub_out = ci, cs
            except ImportError:
                pass
        if (sub_out or "").strip():
            try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("domain", help="Website domain, e.g. stripe.com")
    parser.add_argument(
        "--api-key",
    ...
    if cli.lead_format:
        lead = domain_to_lead_format_json(
            cli.domain,
            api_key=cli.api_key,
        ...
        out = enrich_company_web_profile(
            cli.domain,
            cli.api_key,
            is_b2b=not cli.no_b2b,
            dynamic_first_scrape=cli.dynamic,
